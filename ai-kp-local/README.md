...
- KP 可修改地点/路线并保存不可变 MapSpec revision，绘制和逐区揭开雾区；玩家地图实时
  叠加仍未揭示的遮罩。
...
- 对抗检定持久保存双方原始检定和服务器裁决，并进入同一后果指纹；场景推进支持
  安全暂停、人类 KP 接管和明确交还，切换理由、乐观版本与顺序审计可重启恢复。
- 玩家手册/线索支持草稿、揭示、撤回、固定和逐席已读，并预留对象关联字段；基础
  contract 评测页可保存秘密揭示轨迹并确定性重放，Debug 台显示进程内延迟、锁等待、
  检索质量与秘密泄露告警。
...
- 对抗检定 API/UI 与后果回接已完成；追逐、战斗轮和完整伤害/理智状态机仍未闭环。
...
- 世界事实账本、独立 `/facts` 工作台与 AI `proposed_facts` 原子审批已经接通；默认
  筛选和人工确认负担仍需真人团验证。
- 场景回合、检定后果与世界补全已受接管模式约束；会话摘要等其他模型入口尚未统一
  接入应用层控制闸门，因此当前不能把该能力解释为“停止所有 AI 调用”。
- 线索关联尚缺前端选择器和同团目标真实性校验；现阶段 UI 只完整支持揭示、撤回、
  固定与逐席已读。
- 调查员已具备跨 Campaign 的安全时间线投影、单活动主线约束、显式平行分支和
  玩家确认的永久里程碑版本；分队路线规划、自由多边形雾区绘制和发布差异预览仍属于
  后续能力。
- 雾区虽然记录创建时的地图 revision，但后续 revision 的继承/重映射策略仍需明确；
  在此之前，改变地图坐标系后应由 KP 复核或重建雾区。
- 当前模拟团是确定性秘密投影 contract runner，不会真正调用行动、检定、NPC、记忆、
  地图和审批服务；完整 golden campaign、隔离数据库重放及模型/量化对比仍待实现。
