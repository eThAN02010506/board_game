...
- 模板实体支持版本化当前状态和变更历史。真人 KP 可通过网页维护，AI 状态候选经审核后
  使用同一提交服务；版本由服务器绑定，冲突整笔回滚。无权威内核的自动模式仍清除持久效果。
- KP 可在规则合同中显式关联模组原文实体，保存为待审核新草稿。来源 ID 经校验后用于
  识别实际物件/人物与规则实体的对应关系；同名不会自动合并。规则效果到模板状态的自动转换仍待接入。
...
- 可选择版本化的通用时代包和聚落类型，把模组已确认地点关联到功能槽位，再列出歧义与
  缺项候选。区域拓扑是可替换的连通图模式，不把“城市—小镇—乡村”等示例硬编码为
  世界规律；职位、内部空间、服务、档案、交通与事件种子复用同一模板契约。人物、组织、
  物件、文档、交通工具、事件和线索载体进一步使用带能力、状态维度与关系槽位的实体原型，
  而不是复制固定角色。模组已有实体也可在不可变 Profile 中显式绑定到聚落功能与原型，
  AI 优先按来源 ID 复用。模型只接收与当前意图相关的已选槽位和确定性校验所需字段；
  新手真人 KP 仍可查看完整 Profile。模板结果
  只读且保留来源，未经审批和实际接触不会成为世界事实。
...
- 将 AI 回合限制为结构化草稿；只有人类 KP 批准后，事件、记忆、NPC 更新、检定和
  地图移动才会原子写入正式状态。
- AI KP 与人类 KP 只在“如何选择契约候选”和可选叙事上不同；选定 primitive 后统一
  进入来源无关的 `SelectedKernelAction` 准备边界，再由同一 `ActionResolutionKernel`、
  玩家确认和原子提交链路处理。模型大小或主持模式不会改变可行性、技能白名单、效果
  上限、骰子和世界命令权限。task method 的首步/后续步与多人并行准备、改技能重验、
  入库前重验也必须经过 platform 层同一纯函数，持久层不再自行拼装 preview 或确定性叙事。
  该函数只接收 `SelectedOperator`（operator ID + 可选技能），模型的 route/confidence 留在
  语义与审计层，不渗入规则权限。新手 KP 可从同一受约束候选目录选择，也可使用只读且带
  来源的 Need Help 建议；建议本身不执行行动或写入状态。每次请求采用追加式 requested/
  terminal 审计，终态与安全错误码严格配对；进程中断留下的 requested 会在下次启动追加
  `failed/request_abandoned`，不篡改旧事件。LAN 模式也把该入口纳入敏感操作限流。
- 玩家发言先经过规则集无关的桌面语义协议。对“眼前能看到什么”、已宣称完成的结果和
  第一人称明确交涉动作使用高精度确定性护栏；模型连续两次输出坏结构时，只对显式
  “先…再/然后…”计划恢复有序步骤，其余输入安全要求澄清。护栏只决定会话/信息/角色扮演/
  机械/澄清路由，不选择结果、不写状态，也不因模型更大而放宽权限。
- 单行动、并行规划/结算、真人 KP 手动候选与 Need Help 统一从冻结的
  `ScenarioAuthorityContext` 读取 run、合同绑定、剧透范围与 snapshot。模型等待结束后必须
  重验同一身份；任何场景或证据范围漂移都会丢弃旧结果而不写 proposal。
- AI 语义选择、新手 KP 候选目录与 Need Help 还共用纯 `ScenarioActionCatalog`：候选 ID、
  当前可用性、技能白名单和 contract source refs 只有一套生成逻辑。检索到的模组/规则书
  原文仍标记为补充上下文，不会因为被模型引用而取得执行权限。
- 单行动与多人并行结算保留各自的事务编排，但共用 `KernelAuthorityBasis` 和
  `resolved_action_commands`：回执都冻结 run、有效合同 hash、state version 与待提交 artifact
  hash，每个 primitive 的 `action_resolved` 事件和 outcome 命令只由一个纯函数生成。单行动
  还在同一写事务内重验 action、proposal、campaign 与 actor 身份。
- 新生成的单行动裁决使用 `kernel-resolution.v3`，把 run、合同版本与哈希、状态版本和
  preview 哈希冻结为同一权限依据；原子提交会对当前 binding/state 再验证并把依据写入回执。
  缺少该依据的历史待提交载荷必须重新裁决，不能静默沿用旧权限。
...
`rulebook` 组显式固定本项目调用的 MiniRAG、JSON 修复、NanoVectorDB 与 NumPy，并补齐
该 MiniRAG 固定提交因打包元数据不完整而不会被现代 pip 可靠解析的运行时依赖子集。
若要显著缩小 Steam 基础包，应保持该组与客户端分离或替换索引适配器；不能只删依赖
声明，因为那会把安装期问题延迟为运行期故障。
...
- 模组是带来源的权威大纲，而非完整世界清单。AI 可以提出合理世界补全，但不能覆盖
  Canon、剧情锚点或已确认事实。
- 时代/地域/实体模板只提供候选结构，不提供身份、线索答案、模组真相或规则权限；规则插件、Setting Pack 和
  模组内容彼此独立版本化，因此增加新规则书或年代不会复制 AI KP/人类 KP 执行内核。
...
- 后端：全量运行 2388 项通过、53 个子测试通过、1 项跳过；唯一环境失败是沙箱禁止图片
  provider 测试绑定 localhost，该项随后在允许回环绑定的隔离环境中通过，因此本轮有效结果为
  2389 项通过、1 项跳过。
- 前端：52 个 Vitest 文件、259 项测试在允许 loopback、单 worker 环境全部通过；TypeScript
  与 Vite 生产构建通过。
- 浏览器 real-case：OldOnes 测试团完成公开 D100 投掷；总值、成功等级、逐骰明细、
  服务器时间和 SHA-256 结果指纹均可见并与持久化记录一致。
- 局域网 GPT-OSS-20B MXFP4：模型发现、Kernel 候选/多步计划和桌面语义协议真实调用通过；
  坏结构与不受支持字段由闭合校验/修复拒绝或收敛，结果叙事失败时安全使用确定性提示。
- 同一 20B 在 2200 token 输出上限下完成世界实体纵向链路：服务器固定 `environment`
  工作流，模型经一次有界修复生成“警探 → agency → 小镇警察局”，审批后由 AI/真人 KP 共用
  物化器原子写入实体与关系，下一回合可召回整个可见的一跳关系图；玩家视角不会通过关系摘要
  泄露 KP/secret 实体。
- 四玩家真实 UI 已完成《鬼屋》的建团、入席、建卡审批、Session 0 与整本合同生成；该未微调
  20B 的首次编译约 55 分钟且最终被独立审核拒绝，因而没有发布或进入行动结算。这是明确的
  fail-closed/性能证据，不是完整 Session 或 AC-LONG 通过。
- 该失败 checkpoint 的复盘已修复连续 canonical travel ID 冲突只收缩一次的问题；旧的未发布
  checkpoint 会按新的 authoring authority revision 续跑。新装配器会从显式编号 scene 标题
  确定性物化地点，拒绝把展示材料、属性标题、主持说明或效果段落当地点，并在删地点时收缩所有
  类型化条件依赖。不同楼层的同名房间按来源父级保持分离，不再误作重复项。
- v7 只读重放现得到 15 个来源支持地点、0 个悬空地点条件；编译器与运行时统一按可执行
  `set_scene` 状态探索计算可达性，复用“介绍”作为调查入口，并把明确的“检定成功后交付文字材料”
  编译为同一共享内核中的公开结果和权威事实。文字材料 7 由此可证明地解锁高等法院/中央警察局，
  可达地点为 7 个、仍不可达 8 个。结局和其余线索/路线仍不足，故保持 `release_ready=false`。
  本地 GPT-OSS-20B 的 topology Agent 能返回合法
  JSON，但会误抄来源 ID 或选择未被文本证明的路线；服务器全部拒绝，并提供每批至多一次的
  schema/语义错误反馈重试。该结果是安全失败证据，尚未重复整本 UI 验收。
...
- 世界事实账本、独立 `/facts` 工作台与 AI `proposed_facts` 原子审批已经接通；默认
  筛选和人工确认负担仍需真人团验证。
- 已审核合同可通过 `set_world_entity_state` 把规则效果写入模板实体共享账本；
  单人/并行结算共用事务与幂等收据，AI KP 与真人 KP 不另建状态写入分支。
  自动编制已提供来源实体/维度目录并校验选择，规则条件回读与模型可靠性验证仍待完成，详见
  [规则与模板状态](docs/WORLD_ENTITY_STATE_EFFECTS.md)。
