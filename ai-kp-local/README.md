...
- 模拟团既支持轻量秘密投影 contract，也支持 `product-service-replay.v3`：在 SQLite
...
- 后端：389 项测试与 50 个子测试通过；沙盒禁止监听端口的图片 provider real-case
  需在目标主机单独执行。
- 前端：24 个 Vitest 文件、102 项测试通过；TypeScript 与 Vite 生产构建通过。
- 局域网 GPT-OSS-20B MXFP4：模型发现与结构化 chat completion 通过；gpt-oss 使用
  低 reasoning effort，并拒绝正文为空的截断响应。
- 《常暗之厢》关键 A 结局已通过真实产品服务黄金回放；测试不会把临时角色、地图、
  线索或遭遇写入正式团。
...
- 产品服务回放已有《常暗之厢》关键 A 结局 golden case，覆盖调查员审批、SAN、
  地图/路线、提案、线索和追逐；其失败/战斗/减速结局、NPC 跨团重现、记忆召回、
